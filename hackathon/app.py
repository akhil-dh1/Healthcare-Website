...
# sqlite3 is a alternate light database that you can use instead of using larger databases like MySQL etc
# for using MySQL use flask_mysqldb | Flask-MySQLdb provides MySQL connection for Flask. 
# accessing DATABASE variable from connection file
DATABASE = cn.DATABASE
cn.get_db()

#https://stackoverflow.com/questions/13927643/how-to-give-the-path-for-template-directory
# pyhton variable __file__ automatically set to the file name of the Python module in which the code lives
...
# the different_platform function is called from the index.html file when ask for symptoms
@app.route('/redirecting',methods=['POST','GET'])
def different_platform():
   print("=========================================================================")
   # to access form data POST/PUT
...
def find(symptom):
	...
	# tuple is returned, 1 goes to massage and ans array goes to check1
	massage,check1=healthline.healthline_web_scraping(symptom)
	   # print(check1[0])
	if len(check1)>=3:
		return render_template('index.html',msg="For your given symtom "+symptom+" these are top three result:-",msg1=check1[0],msg2=check1[1],msg3=check1[2])
	 	...
	 	return render_template('index.html',msg="For your given symtom "+symptom+" these are top three result:-",msg1="",msg2="",msg3="")
	 # print(massage)
...
# app.run(debug = True) helps in debugging by: tracebacks are also printed to the terminal running the server, regardless of development mode.
